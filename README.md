...
> **Version 0.1.2 — Stable bug-fix release.** Core engine is solid with rollback, dry-run, and safety features. See [CHANGELOG.md](./CHANGELOG.md) and [BENCHMARK.md](./BENCHMARK.md) for details.
...
- **Heuristic Command Guard**: Dangerous commands (`rm -rf /`, `git push --force`, `curl | sh`, etc.) are detected and can be blocked/approved per policy. The guard is a warning layer, not a sandbox — see [SECURITY.md](./SECURITY.md)
