...
class ExecutionContext {
  ...
  addStepResult(index, result) {
    ...
    // Also index by explicit step id if present
    if (result._step_id) {
...
function evaluateCondition(condition, context) {
  ...
  const containsMatch = partial.match(/\b(\w+)\[(\d+)\]\.stdout_contains\("(.+?)"\)/);
  if (containsMatch) {
  ...
  const idContainsMatch = partial.match(/\bsteps\.(\w+)\.stdout_contains\("(.+?)"\)/);
  if (idContainsMatch) {
  ...
  // v0.1.4: Detect unresolved step references (typo property names, missing steps)
  // After resolveString, any remaining step[N].xxx or steps.X.yyy pattern means
  // the reference could not be resolved → invalid expression.
  if (/\b(?:step\[\d+\]\.\w+|steps\.\w+\.\w+)/.test(resolved)) {
    return {
      valid: false,
      value: false,
      error: `Unresolved step reference in condition: "${resolved.trim()}". ` +
...
function executeConditional(step, context, stopOnError) {
  ...
  for (let i = 0; i < branch.length; i++) {
    ...
    // v0.1.4: Branch results are NOT added to context.stepResults.
    // They are accessible via the conditional result's branch_results field.
    // Use explicit step IDs for cross-branch data references.
...
export function runMacro(steps, options = {}) {
  ...
  // Recursive leaf step counting (includes steps inside conditional branches)
  const leafCounts = countLeafSteps(results);
  ...
  // Token savings estimate (use leaf counts for accuracy)
  const tokenEstimate = estimateTokenSavings(leafCounts.total, leafCounts.executed, results);
  ...
  return {
    ...
    leaf_steps_executed: leafCounts.executed,
    leaf_steps_total: leafCounts.total,
    total_steps: steps.length,
...
/**
 * Count leaf steps recursively (non-conditional steps that actually execute work).
 */
function countLeafSteps(results) {
  ...
  function walk(steps) {
    for (const step of steps) {
      ...
      if (step.branch_results && Array.isArray(step.branch_results)) {
        // Don't double-count the conditional itself if it executed
        executed--; total--;
  ...
  // For non-executed conditional steps (dry run, approval_required, etc.),
  // also count their planned branch steps
  return { executed: Math.max(0, executed), total: Math.max(0, total) };
}
...
function estimateTokenSavings(totalLeafSteps, executedLeafSteps, results) {
