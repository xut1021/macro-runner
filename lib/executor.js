...
import { readFileSync, writeFileSync, existsSync, appendFileSync, realpathSync, statSync } from 'fs';
import { spawnSync } from 'child_process';
import { resolve, normalize, sep, dirname, basename, join } from 'path';
import { CONFIG, DANGEROUS_ENV_KEYS, isDangerousEnvKey } from './config.js';
...
function safeResolve(filePath) {
  ...
  if (CONFIG.workspaceRoot && !CONFIG.allowOutsideWorkspace) {
    // v0.1.6: realpath the workspace root itself for consistent comparison.
    // Fail-closed: if realpath fails, the root path is untrusted → reject.
    let realRoot;
    try {
...
class ExecutionContext {
  ...
  remainingMs(totalTimeoutMs) {
    // v0.1.6: only null/undefined means "no limit"; 0 means already expired
    if (totalTimeoutMs === null || totalTimeoutMs === undefined) return Infinity;
...
/**
 * v0.1.8: Verify a path points to a regular file (not FIFO, socket, device).
 * statSync throws for non-existent paths — caller must check existsSync first.
 */
function checkRegularFile(filePath) {
  try {
    const stat = statSync(filePath);
    if (!stat.isFile()) {
      return { ok: false, error: `Not a regular file: ${filePath}. Only regular files are supported.` };
    }
  } catch (_) {
    // statSync failed — file may not exist (caller handles this)
  }
  return { ok: true };
}
...
function executeEdit(step) {
  ...
  // v0.1.8: Reject non-regular files (FIFO, socket, device) and oversized files
  const fileCheck = checkRegularFile(filePath);
  if (!fileCheck.ok) return fileCheck;

  let fileBytes;
  try { fileBytes = statSync(filePath).size; } catch (_) { fileBytes = 0; }
  if (fileBytes > CONFIG.maxEditFileBytes) {
    return {
      ok: false,
      error: `File too large to edit: ${rawPath} (${(fileBytes / 1024 / 1024).toFixed(1)} MB). ` +
        `Max: ${CONFIG.maxEditFileBytes / 1024 / 1024} MB. Set MACRO_MAX_EDIT_FILE_BYTES to increase.`,
    };
  }
...
function executeShell(step, remainingTimeout) {
  ...
  // v0.1.11: Case-insensitive execution-control env var blocking
  ...
  if (blockedEnvKeys.length > 0) {
    const mode = CONFIG.dangerousCommandsMode;
    if (mode === 'deny') {
      return {
        ok: false,
        policy_denied: true,
        error: `Execution-control env var(s) blocked by policy: ${blockedEnvKeys.join(', ')}. ` +
          'These can change program behavior and bypass command audit.',
      };
    }
    if (mode === 'approve') {
      return {
        ...
        risk: 'high',
        reasons: blockedEnvKeys.map(k => `[HIGH] Execution-control env var: ${k}`),
        dangerous_env_keys: blockedEnvKeys,
        error: `Execution-control env var(s) require approval: ${blockedEnvKeys.join(', ')}. ` +
          'Set MACRO_DANGEROUS_COMMANDS=warn to bypass.',
      };
    }
    // warn mode: allow but record warning (v0.1.12: actually record)
    if (blockedEnvKeys.length > 0) {
      // mode === 'warn' — continue execution, attach warnings to result later
    }
  }

  // Collect security warnings for warn mode
  let securityWarnings = null;
  if (blockedEnvKeys.length > 0 && CONFIG.dangerousCommandsMode === 'warn') {
    securityWarnings = blockedEnvKeys.map(k => `[HIGH] Execution-control env var: ${k}`);
  }
  ...
  // Enforce per-channel output size limits (v0.1.7: safe UTF-8 boundary + marker budget)
  ...
  function truncateUTF8(buf, maxBytes) {
    // v0.1.9: Only truncate if actually exceeds limit
    if (buf.length <= maxBytes) {
      return { text: buf.toString('utf8'), truncated: false, omitted: 0 };
    }
    // Reserve bytes for truncation marker based on actual marker length
    const marker = `\n[... truncated: ${buf.length - maxBytes} bytes omitted ...]`;
    const markerBytes = Buffer.byteLength(marker, 'utf8');
    const effectiveMax = Math.max(1, maxBytes - markerBytes);
    // Walk back from cut point to valid UTF-8 boundary
    let cut = effectiveMax;
    while (cut > 0 && (buf[cut] & 0xC0) === 0x80) cut--;
    const omitted = buf.length - cut;
    return { text: buf.slice(0, cut).toString('utf8'), truncated: true, omitted };
  }

  const stdoutResult = truncateUTF8(stdoutBuf, CONFIG.stdoutMaxBytes);
  stdout = stdoutResult.text;
  if (stdoutResult.truncated) {
    stdout += `\n[... truncated: ${stdoutResult.omitted} bytes omitted ...]`;
    stdoutTruncated = true;
  }

  const stderrResult = truncateUTF8(stderrBuf, CONFIG.stderrMaxBytes);
  stderr = stderrResult.text;
  if (stderrResult.truncated) {
    stderr += `\n[... truncated: ${stderrResult.omitted} bytes omitted ...]`;
    stderrTruncated = true;
  }

  const stdoutOrigBytes = stdoutBuf.length;
  const stderrOrigBytes = stderrBuf.length;
  ...
  if (result.error) {
    ...
    return {
      ...
      security_warnings: securityWarnings || undefined,
      executed_with_warning: securityWarnings ? true : undefined,
      stdout,
...
function executeRead(step) {
  ...
  // v0.1.7: Reject files exceeding read size limit (avoid loading huge files)
  let fileBytes;
  try { fileBytes = statSync(filePath).size; } catch (_) { fileBytes = 0; }
  if (fileBytes > CONFIG.maxReadFileBytes) {
    return {
      ok: false,
      error: `File too large to read: ${rawPath} (${(fileBytes / 1024 / 1024).toFixed(1)} MB). ` +
        `Max: ${CONFIG.maxReadFileBytes / 1024 / 1024} MB. Set MACRO_MAX_READ_FILE_BYTES to increase.`,
    };
  }
...
function evaluateCondition(condition, context) {
  // Resolve ${{var}} user-variable references first (callback form — no $& interpretation).
  // Step references are resolved LATER via resolveString, after stdout_contains check.
  ...
  // (resolveString would convert step[N].stdout into a value,
  // making the stdout_contains pattern un-matchable)
  // v0.1.6: anchored patterns, missing steps/IDs return valid:false
  ...
  if (containsMatch) {
    ...
    if (!result) {
      return { valid: false, value: false, error: `Unknown step index ${index} in stdout_contains` };
    }
    // v0.1.6: empty pattern matches everything (JS includes("") === true)
    const stdout = result.stdout ?? '';
    return { valid: true, value: stdout.includes(pattern) };
  }
  ...
  if (idContainsMatch) {
    ...
    if (!result) {
      return { valid: false, value: false, error: `Unknown step ID "${id}" in stdout_contains` };
    }
    const stdout = result.stdout ?? '';
    return { valid: true, value: stdout.includes(pattern) };
  }
...
function executeConditional(step, context, stopOnError) {
  ...
  for (let i = 0; i < branch.length; i++) {
    ...
    // v0.1.11: Build nested execution path BEFORE executing sub-step
    const parentPath = step._execution_path || `steps[${step._parent_index}]`;
    const branchPath = `${parentPath}.${conditionMet ? 'then' : 'else'}[${i}]`;
    const result = executeStep(subStep, context, remaining, step._rollback, stopOnError, branchPath);
    ...
    if (!result.ok) {
      // v0.1.11: timeout, policy_denied, approval, or stopOnError — always stops
      if (result.timed_out || result.policy_denied || result.approval_required || stopOnError) {
        return {
          ok: false,
          condition_met: conditionMet,
          error: result.error || `Conditional branch sub-step ${i} failed`,
          timed_out: result.timed_out === true || undefined,
          approval_required: result.approval_required || undefined,
          policy_denied: result.policy_denied || undefined,
          summary: `Conditional ${conditionMet ? 'then' : 'else'} branch ${result.timed_out ? 'timed out' : 'stopped'} at sub-step ${i}: ${result.error || 'unknown error'}`,
          branch_results: results,
        };
      }
      // stop_on_error: false — record failure and continue branch (unless timed out or approval)
    }
  }
 ...
 */
function _executeStepImpl(step, context, remainingMs, rollback, stopOnError, executionPath) {
  // v0.1.6: deadline check BEFORE any side effects (edit/write/shell).
  // Timeout is not overridable by stop_on_error — it always halts.
  if (remainingMs !== undefined && remainingMs !== null && remainingMs <= 0) {
    return {
      ok: false,
      type: step.type || 'unknown',
      description: step.description || '',
      timed_out: true,
      error: 'Macro deadline reached before step execution',
      duration_ms: 0,
    };
  }
  ...
  const RESOLVABLE_FIELDS = ['path', 'file', 'command', 'old_str', 'old_string', 'new_str', 'new_string', 'content', 'message', 'cwd'];
  for (const key of RESOLVABLE_FIELDS) {
    if (typeof step[key] === 'string') {
      resolvedStep[key] = context.resolveString(step[key]);
    }
  }
  // Also resolve env values (v0.1.9)
  if (step.env && typeof step.env === 'object') {
    const resolvedEnv = {};
    for (const [k, v] of Object.entries(step.env)) {
      resolvedEnv[k] = typeof v === 'string' ? context.resolveString(v) : v;
    }
    resolvedStep.env = resolvedEnv;
  }
  ...
  // v0.1.9: Reject unresolved references in ALL string fields (including old_str/new_str/cwd/env).
  // Any UNRESOLVED sentinel means a reference could not be satisfied → fail before side effects.
  const ALL_STRING_FIELDS = [...RESOLVABLE_FIELDS, 'condition'];
  for (const key of ALL_STRING_FIELDS) {
    if (typeof resolvedStep[key] === 'string' && resolvedStep[key].includes(UNRESOLVED)) {
      return {
        ok: false,
        type: resolvedStep.type || 'unknown',
        error: `Unresolved step reference in "${key}" field: "${step[key]}". Check the step index/ID and property name.`,
      };
    }
  }
  // Check env values too
  if (resolvedStep.env) {
    for (const [k, v] of Object.entries(resolvedStep.env)) {
      if (typeof v === 'string' && v.includes(UNRESOLVED)) {
        return {
          ok: false,
          type: resolvedStep.type || 'unknown',
          error: `Unresolved step reference in env.${k}: "${step.env[k]}".`,
        };
      }
    }
  }

  // v0.1.9: File safety preflight — BEFORE rollback snapshot.
  // If the target is a FIFO/device/socket, rollback.snapshot()'s readFileSync would block.
  if (resolvedStep.type === 'edit' || resolvedStep.type === 'write' || resolvedStep.type === 'read') {
  ...
  // ---- Rollback snapshot (AFTER safety checks pass) ----
  ...
  // Pass absolute deadline, rollback, stop_on_error, and execution path to conditional branches
  if (resolvedStep.type === 'conditional') {
    ...
    // v0.1.11: carry execution path for nested branch path construction
    resolvedStep._parent_index = step._parent_index;
    resolvedStep._execution_path = executionPath;
  }
  ...
  switch (resolvedStep.type) {
    ...
    case 'write':
      ...
      break;
    case 'shell':
      // v0.1.7: Re-audit resolved command after variable substitution.
      // Dynamic commands (e.g. ${{cmd}} reading from a file) bypass the
      // static pre-audit, so we must re-check the resolved command.
      if (resolvedStep.command !== (step.command || '')) {
        const runtimeAudit = checkCommand(resolvedStep.command);
        if (!runtimeAudit.approved) {
          const mode = CONFIG.dangerousCommandsMode;
          if (mode === 'deny') {
            result = {
              ok: false,
              policy_denied: true,
              risk: runtimeAudit.risk,
              reasons: runtimeAudit.reasons,
              original_command: step.command,
              resolved_command: resolvedStep.command,
              error: `Runtime command re-audit denied by policy: "${resolvedStep.command}" (risk: ${runtimeAudit.risk}).`,
            };
          } else {
            result = {
              ok: false,
              ...
              original_command: step.command,
              resolved_command: resolvedStep.command,
              error: `Runtime command re-audit requires approval: "${resolvedStep.command}" (risk: ${runtimeAudit.risk}). Set MACRO_DANGEROUS_COMMANDS=warn to bypass.`,
            };
          }
          break;
        }
  ...
  // v0.1.11: execution path set BEFORE return (not after)
  result._execution_path = executionPath || '';
  ...
  // v0.1.11: Capture identity of newly created files AFTER write succeeds
  if (rollback && result.ok && (resolvedStep.type === 'write' || resolvedStep.type === 'edit')) {
    const rawPath = resolvedStep.path || resolvedStep.file;
    if (rawPath && typeof rawPath === 'string') {
      const fp = safeResolve(rawPath);
      if (fp && existsSync(fp)) {
        rollback.captureCreatedIdentity(fp);
      }
    }
  }
 ...
 */
function executeStep(step, context, remainingMs, rollback, stopOnError, executionPath) {
  try {
    return _executeStepImpl(step, context, remainingMs, rollback, stopOnError, executionPath);
  } catch (error) {
    return {
      ok: false,
      type: step.type || 'unknown',
...
export function runMacro(steps, options = {}) {
  ...
  // v0.1.6: default 300000ms (must be explicit null to disable)
  const macroTimeoutMs = options.timeout_ms ?? 300000;
  ...
  if (danger && !danger.approved) {
    ...
    return {
      status: isDeny ? 'policy_denied' : 'approval_required',
      execution_status: isDeny ? 'policy_denied' : 'approval_required',
      run_id: runId,
      risk: danger.risk,
      reasons: danger.reasons,
      blocked_command: danger.command,
      blocked_at_step: danger.step_index,
      approval_required: !isDeny || undefined,
      policy_denied: isDeny || undefined,
      executed_steps: 0,
      passed_steps: 0,
      failed_steps: 0,
      total_steps: steps.length,
      ...
      steps: [],
      hint: isDeny
        ? 'Dangerous command blocked by policy (deny mode). Set MACRO_DANGEROUS_COMMANDS=approve to require approval, or =warn to allow with warning.'
        : `Dangerous command requires approval. Set MACRO_DANGEROUS_COMMANDS=warn to bypass, or =deny to block all dangerous commands. Currently: ${mode}`,
    };
  }
  ...
  // ---- Phase 2: Execute (with try/catch/finally for guaranteed rollback) ----
  ...
  try {
    for (let i = 0; i < steps.length; i++) {
      ...
      const execPath = `steps[${i}]`;
      step._parent_index = i;
      const result = executeStep(step, context, remaining, rollback, stopOnError, execPath);
      result.index = i;
      if (!result._execution_path) result._execution_path = execPath;
      results.push(result);
      ...
      // v0.1.12: policy_denied and approval_required are independent
      if (result.policy_denied) {
        ...
        approvalDetails = {
          ...
          blocked_command: result.resolved_command || result.original_command,
        };
        if (firstFailure < 0) firstFailure = i;
        break;
      }
      ...
      if (result.approval_required) {
        ...
        policyDenied = false; // mutually exclusive at step level
        approvalStep = i;
        approvalDetails = {
          policy: 'approve',
          risk: result.risk,
    ...
    // v0.1.6: Unexpected exception escaping the loop → always trigger rollback.
    // Without this, macroFailed stays false and rollback.clear() would discard snapshots.
    macroFailed = true;
    fatalError = error instanceof Error ? error.message : String(error);
  } finally {
    // ---- Phase 3: Rollback on failure — ALWAYS runs (v0.1.4, hardened v0.1.6) ----
  ...
  const leafDeclared = countDeclaredLeaves(steps);
  ...
  } else if (rolledBack && (approvalRequired || policyDenied)) {
    // v0.1.12: rollback succeeded but execution was blocked by policy
    status = 'rolled_back';
  } else if (rolledBack) {
    status = 'rolled_back';
  } else if (fatalError) {
  ...
  if (fatalError) execution_status = 'internal_error';
  else if (policyDenied) execution_status = 'policy_denied';
  else if (approvalRequired) execution_status = 'approval_required';
  else if (timedOut) execution_status = 'timed_out';
  ...
  const tokenEstimate = estimateTokenSavings(leafDeclared, leafExecuted, results);
  ...
  return {
    ...
    leaf_steps_declared: leafDeclared,
    leaf_steps_executed: leafExecuted,
    leaf_steps_not_reached: Math.max(0, leafDeclared - leafExecuted),
    total_steps: steps.length,
    ...
    policy_denied: policyDenied || undefined,
    approval_details: approvalDetails || undefined,
    fatal_error: fatalError || undefined,
...
function dryRunPreview(step, context, index) {
  ...
  switch (resolvedStep.type) {
    ...
    case 'shell': {
      ...
      const rawCmd = resolvedStep.command || '(missing)';
      preview.command = rawCmd;
      // v0.1.10: Mark dynamic commands as risk_unknown (can't statically audit)
      const hasUnresolvedVars = /\$\{\{/.test(rawCmd);
      if (rawCmd !== (step.command || '') || rawCmd.includes(UNRESOLVED) || hasUnresolvedVars) {
        preview.risk = 'unknown';
        preview.dynamic = true;
        preview.needs_runtime_audit = true;
      } else {
        const check = checkCommand(rawCmd);
      ...
      // v0.1.11: Audit env using shared danger key list (case-insensitive)
      const stepEnv = step.env || {};
      const dangerousEnvKeys = Object.keys(stepEnv).filter(k => isDangerousEnvKey(k));
      if (dangerousEnvKeys.length > 0) {
        preview.dangerous_env_keys = dangerousEnvKeys;
        if (preview.risk === 'low') preview.risk = 'medium';
        preview.needs_approval = CONFIG.dangerousCommandsMode !== 'warn';
      }
      break;
    }
    case 'read':
 ...
 */
function countDeclaredLeaves(steps) {
  let total = 0;
...
function estimateTokenSavings(totalLeafDeclared, executedLeafSteps, results) {
  const executed = Math.max(executedLeafSteps, 1);
  const shellSteps = countShellSteps(results);

  // v0.1.8: Single-step macros don't save round trips → no token savings
  if (executedLeafSteps <= 1) {
    return {
      estimated_tokens_without_macro: 1500,
      tokens_consumed: 1500,
      tokens_saved: 0,
      round_trips_saved: 0,
      potential_tokens_saved: (totalLeafDeclared > 1) ? (totalLeafDeclared - 1) * 975 : 0,
      potential_round_trips_saved: Math.max(0, totalLeafDeclared - 1),
      _note: 'Single-step macro: no round-trip savings. potential_tokens_saved reflects multi-step case.',
    };
  }

  // v0.1.7: Split into observed (based on what actually ran) and potential
  // (based on declared steps including untaken branches).

  // Observed estimate — only counts steps that actually executed
  const obsWithout = (executed * 1500) + (shellSteps * 500);
  const obsSaved = Math.max(0, executed - 1);
  const obsWith = Math.floor(obsWithout * 0.35);

  // Potential estimate — what the full macro would save if all branches ran
  const potWithout = (totalLeafDeclared * 1500) + (shellSteps * 500);
  const potSaved = Math.max(0, totalLeafDeclared - 1);
  const potWith = Math.floor(potWithout * 0.35);

  return {
    estimated_tokens_without_macro: obsWithout,
    tokens_consumed: obsWith,
    tokens_saved: obsWithout - obsWith,
    round_trips_saved: obsSaved,
    potential_tokens_saved: potWithout - potWith,
    potential_round_trips_saved: potSaved,
    _note: 'Experimental heuristic estimate. "tokens_saved" is based on actual executed steps; "potential_tokens_saved" includes all declared steps.',
  };
}
...
export function logBenchmark(macroResult) {
  // v0.1.6: Always track session counters by category
  ...
  // Categorize (v0.1.7: dry_run checked FIRST — dry-run status is 'completed' too)
  const status = macroResult.status;
  if (macroResult.dry_run) {
    ...
    totalBlocked++;
  } else if (status === 'internal_error' || status === 'failed_early' ||
             status === 'completed_with_failures' || status === 'timed_out' ||
             status === 'rolled_back' || status === 'rollback_failed') {
    totalFailed++;
  }
  ...
  const entry = {
    ...
    execution_status: macroResult.execution_status || undefined,
    rollback_status: macroResult.rollback_status || undefined,
    timed_out: macroResult.timed_out || undefined,
    dry_run: macroResult.dry_run || undefined,
    ...
    leaf_steps_declared: macroResult.leaf_steps_declared,
    leaf_steps_executed: macroResult.leaf_steps_executed,
...
export function getStats() {
  return {
    ...
    failed: totalFailed,
    dry_runs: totalDryRuns,
    approval_blocked: totalBlocked,
    validation_failed: totalValidationFailed,
