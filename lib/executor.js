...
function executeConditional(step, context) {
  ...
  // Execute branch steps sequentially (inherit timeout and rollback from parent)
  const results = [];
  let firstBranchError = null;
  for (let i = 0; i < branch.length; i++) {
    const subStep = branch[i];
    // Use remaining time from parent context, not Infinity
    const remaining = step._remainingMs !== undefined ? step._remainingMs : Infinity;
    ...
    if (!result.ok && !firstBranchError) {
      firstBranchError = result;
    }
    if (!result.ok) {
      return {
        ok: false,
...
function executeStep(step, context, remainingMs, rollback) {
  ...
  // Pass timeout to conditional branches
  if (resolvedStep.type === 'conditional') {
    resolvedStep._remainingMs = remainingMs;
