...
function executeConditional(step, context) {
  ...
  for (let i = 0; i < branch.length; i++) {
    ...
    const result = executeStep(subStep, context, remaining, step._rollback);
 ...
 */
function executeStep(step, context, remainingMs, rollback) {
  ...
  // Pass absolute deadline and rollback to conditional branches
...
export function runMacro(steps, options = {}) {
  ...
  for (let i = 0; i < steps.length; i++) {
    ...
    const result = executeStep(step, context, remaining, rollback);
    result.index = i;
    results.push(result);
