...
function executeShell(step, remainingTimeout) {
  ...
  // Enforce per-channel output size limits (v0.1.6: true byte-level truncation)
  ...
  if (stdoutBuf.length > CONFIG.stdoutMaxBytes) {
    const omitted = stdoutBuf.length - CONFIG.stdoutMaxBytes;
    stdout = stdoutBuf.slice(0, CONFIG.stdoutMaxBytes).toString('utf8');
    stdout += `\n[... truncated: ${omitted} bytes omitted ...]`;
    stdoutTruncated = true;
  }
  if (stderrBuf.length > CONFIG.stderrMaxBytes) {
    const omitted = stderrBuf.length - CONFIG.stderrMaxBytes;
    stderr = stderrBuf.slice(0, CONFIG.stderrMaxBytes).toString('utf8');
    stderr += `\n[... truncated: ${omitted} bytes omitted ...]`;
...
function estimateTokenSavings(totalLeafDeclared, executedLeafSteps, results) {
  // Base estimate on declared steps (what the macro WOULD do if fully executed)
  // but cap at actual executed for partial runs
  const baseline = Math.max(totalLeafDeclared, executedLeafSteps, 1);
  const shellSteps = countShellSteps(results);

  // Conservative estimate: each individual round trip ≈ 1500 tokens
  const estimatedWithoutMacro = (baseline * 1500) + (shellSteps * 500);
  const roundTripsSaved = Math.max(0, baseline - 1);
...
export function logBenchmark(macroResult) {
  ...
  else if (status === 'validation_failed') totalValidationFailed++;
  else if (macroResult.dry_run) totalDryRuns++;
  else if (status === 'approval_required') totalBlocked++;
  else if (status === 'failed_early' || status === 'completed_with_failures' ||
           status === 'timed_out' || status === 'rolled_back' || status === 'rollback_failed') {
    totalFailed++;
  }
