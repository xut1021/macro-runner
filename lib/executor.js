...
// v0.1.8: Parse numeric env vars safely — NaN defaults to the fallback value
...
const CONFIG = {
  get defaultTimeout() { return safeParseInt('MACRO_DEFAULT_TIMEOUT_MS', 300000); },
  get defaultOutputMode() { return process.env.MACRO_DEFAULT_OUTPUT_MODE || 'summary'; },
  get trimOutputLines() { return safeParseInt('MACRO_TRIM_OUTPUT_LINES', 200); },
  ...
  get stdoutMaxBytes() { return safeParseInt('MACRO_STDOUT_MAX_BYTES', 524288); },
  get stderrMaxBytes() { return safeParseInt('MACRO_STDERR_MAX_BYTES', 524288); },
  get dangerousCommandsMode() {
    const raw = process.env.MACRO_DANGEROUS_COMMANDS || 'approve';
    // v0.1.10: Validate mode at access time — unknown values fail closed
    const VALID_MODES = new Set(['deny', 'approve', 'warn']);
  ...
  get maxRollbackFileBytes() { return safeParseInt('MACRO_MAX_ROLLBACK_FILE_BYTES', 10485760); },
  get maxRollbackTotalBytes() { return safeParseInt('MACRO_MAX_ROLLBACK_TOTAL_BYTES', 104857600); },
  get maxReadFileBytes() { return safeParseInt('MACRO_MAX_READ_FILE_BYTES', 52428800); },
  get maxEditFileBytes() { return safeParseInt('MACRO_MAX_EDIT_FILE_BYTES', 10485760); },
};
...
function executeShell(step, remainingTimeout) {
  ...
  // v0.1.8: Block execution-control env vars that can change program behavior
  const DANGEROUS_ENV_KEYS = [
    'NODE_OPTIONS', 'PATH', 'PATHEXT', 'COMSPEC', 'SHELL', 'BASH_ENV', 'ENV',
    'PYTHONPATH', 'PYTHONSTARTUP', 'RUBYOPT', 'PERL5OPT',
    'LD_PRELOAD', 'LD_LIBRARY_PATH', 'DYLD_INSERT_LIBRARIES', 'DYLD_LIBRARY_PATH',
    'GIT_SSH_COMMAND', 'GIT_EXEC_PATH', 'SSH_AUTH_SOCK',
  ];
  const stepEnv = step.env || {};
  const blockedEnvKeys = DANGEROUS_ENV_KEYS.filter(k => k in stepEnv);
  if (blockedEnvKeys.length > 0) {
    const mode = CONFIG.dangerousCommandsMode;
    if (mode === 'deny') {
      return {
        ok: false,
        error: `Execution-control env var(s) blocked: ${blockedEnvKeys.join(', ')}. ` +
...
function executeConditional(step, context, stopOnError) {
  ...
  for (let i = 0; i < branch.length; i++) {
    ...
    // v0.1.7: Save stable execution path for audit traceability
    const parentPath = step._execution_path || `steps[${step._parent_index}]`;
    result._execution_path = `${parentPath}.${result._branch_name}[${i}]`;
    results.push(result);
    ...
    if (!result.ok) {
      // v0.1.9: timeout, approval, or stopOnError — always stops the branch
      if (result.timed_out || result.approval_required || stopOnError) {
        return {
          ok: false,
          condition_met: conditionMet,
...
function _executeStepImpl(step, context, remainingMs, rollback, stopOnError) {
  ...
  if (resolvedStep.type === 'conditional') {
    ...
    // v0.1.7: carry execution path for nested branch path construction
    resolvedStep._parent_index = step._parent_index;
    resolvedStep._execution_path = step._execution_path;
  }
      ...
      if (resolvedStep.command !== (step.command || '')) {
        ...
        if (!runtimeAudit.approved) {
          result = {
            ...
            error: `Runtime command re-audit blocked: "${resolvedStep.command}" (risk: ${runtimeAudit.risk}). Set MACRO_DANGEROUS_COMMANDS=warn to bypass, or use an approval_token.`,
          };
          break;
        }
      }
...
export function runMacro(steps, options = {}) {
  ...
  try {
    for (let i = 0; i < steps.length; i++) {
      ...
      // v0.1.7: pass parent index for execution path tracking in branches
      step._parent_index = i;
      const result = executeStep(step, context, remaining, rollback, stopOnError);
      result.index = i;
      result._execution_path = `steps[${i}]`;
      results.push(result);
      ...
      // v0.1.10: approval_required always stops and tracks details
  ...
  } else if (rolledBack && approvalRequired) {
    // v0.1.10: rollback succeeded but execution was blocked by approval
    status = 'rolled_back';
  } else if (rolledBack) {
    status = 'rolled_back';
  } else if (fatalError) {
...
function dryRunPreview(step, context, index) {
  ...
  switch (resolvedStep.type) {
    ...
    case 'shell': {
      ...
      // v0.1.10: Audit env for execution-control variables
      const stepEnv = step.env || {};
      const DANGEROUS_ENV = ['NODE_OPTIONS','PATH','PATHEXT','LD_PRELOAD','LD_LIBRARY_PATH',
        'DYLD_INSERT_LIBRARIES','PYTHONPATH','PYTHONSTARTUP','RUBYOPT','PERL5OPT',
        'GIT_SSH_COMMAND','COMSPEC','SHELL','BASH_ENV'];
      const dangerousEnvKeys = DANGEROUS_ENV.filter(k => k in stepEnv || k.toUpperCase() in stepEnv);
      if (dangerousEnvKeys.length > 0) {
        preview.dangerous_env_keys = dangerousEnvKeys;
        preview.risk = preview.risk === 'low' ? 'medium' : preview.risk;
      }
      break;
    }
    case 'read':
