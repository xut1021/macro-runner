...
function validateStep(step, path = '', assignedVars = new Set(), stepIds = new Set()) {
  ...
  // v0.1.9: Enforce field types — numbers must be finite integers where applicable
  const TYPE_CHECKS = {
    path: 'string',
    file: 'string',
    command: 'string',
  ...
  for (const [field, expectedType] of Object.entries(TYPE_CHECKS)) {
    if (step[field] !== undefined && step[field] !== null && typeof step[field] !== expectedType) {
      errors.push(`${prefix}: "${field}" must be ${expectedType}, got ${typeof step[field]}`);
    }
  }

  // v0.1.9: Integer fields must be finite, safe integers
  const INTEGER_FIELDS = ['timeout_ms', 'trim_output_lines', 'offset', 'limit'];
  for (const field of INTEGER_FIELDS) {
    const val = step[field];
    if (val !== undefined && val !== null) {
      if (typeof val !== 'number' || !Number.isFinite(val)) {
        errors.push(`${prefix}: "${field}" must be a finite number, got ${val}`);
      } else if (!Number.isInteger(val)) {
        errors.push(`${prefix}: "${field}" must be an integer, got ${val}`);
      }
    }
  }
  ...
  if (step.env !== undefined && step.env !== null) {
    if (typeof step.env !== 'object' || Array.isArray(step.env)) {
      errors.push(`${prefix}: "env" must be a plain object, got ${typeof step.env}`);
    } else {
      for (const [k, v] of Object.entries(step.env)) {
        if (typeof k !== 'string' || k === '') {
          errors.push(`${prefix}: env keys must be non-empty strings, got "${k}"`);
        }
        if (typeof v !== 'string') {
          errors.push(`${prefix}: env.${k} must be a string, got ${typeof v}`);
        }
      }
    }
  }
...
export function validateMacro(steps, macroOptions = {}) {
  ...
  function countAll(stepList) {
    for (const s of stepList) {
      if (!s || typeof s !== 'object') { recursiveTotal++; continue; }
      recursiveTotal++;
