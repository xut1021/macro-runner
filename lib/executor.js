...
function executeShell(step, remainingTimeout) {
  ...
  if (blockedEnvKeys.length > 0) {
    ...
    // warn mode: allow but record warning
  }
...
export function runMacro(steps, options = {}) {
  ...
  try {
    for (let i = 0; i < steps.length; i++) {
      ...
      // v0.1.11: policy_denied and approval_required always stop
  ...
  if (rollbackFailed) {
    ...
    // v0.1.11: policy_denied is distinct from approval_required
    status = (approvalDetails && approvalDetails.policy === 'deny')
  ...
  if (fatalError) execution_status = 'internal_error';
    ...
    // v0.1.11: distinguish policy_denied from approval_required
    execution_status = (approvalDetails && approvalDetails.policy === 'deny')
      ? 'policy_denied' : 'approval_required';
  }
