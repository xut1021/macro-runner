...
function executeShell(step, remainingTimeout) {
  ...
  if (blockedEnvKeys.length > 0 && CONFIG.dangerousCommandsMode === 'deny') {
    return {
      ok: false,
      error: `Blocked execution-control env var(s): ${blockedEnvKeys.join(', ')}. ` +
        'These can change program behavior and bypass command audit. Set MACRO_DANGEROUS_COMMANDS=warn to allow.',
  ...
  function truncateUTF8(buf, maxBytes) {
    // Reserve ~60 bytes for the truncation marker
    const markerReserve = 60;
    const effectiveMax = Math.max(1, maxBytes - markerReserve);
    if (buf.length <= effectiveMax) return { text: buf.toString('utf8'), truncated: false, omitted: 0 };
...
function executeConditional(step, context, stopOnError) {
  ...
  for (let i = 0; i < branch.length; i++) {
    ...
    if (!result.ok) {
      // v0.1.6: timeout ALWAYS stops (not overridable by stop_on_error)
      if (result.timed_out || stopOnError) {
        return {
          ok: false,
          condition_met: conditionMet,
          error: result.error || `Conditional branch sub-step ${i} failed`,
          timed_out: result.timed_out === true || undefined,
          summary: `Conditional ${conditionMet ? 'then' : 'else'} branch ${result.timed_out ? 'timed out' : 'failed'} at sub-step ${i}: ${result.error || 'unknown error'}`,
          branch_results: results,
        };
      }
      // stop_on_error: false — record failure and continue branch (unless timed out)
    }
  }
...
function _executeStepImpl(step, context, remainingMs, rollback, stopOnError) {
  ...
  // v0.1.8: Reject unresolved references in ALL fields (not just condition).
  // Otherwise we might write to a file named __MACRO_UNRESOLVED__ or run it as a command.
  for (const key of ['path', 'file', 'command', 'content', 'message']) {
