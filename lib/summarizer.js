...
export function formatStepResult(stepResult, mode = 'summary') {
  ...
  // v0.1.12: propagate approval/policy/risk fields at step level
  if (stepResult.approval_required) formatted.approval_required = true;
  if (stepResult.policy_denied) formatted.policy_denied = true;
  if (stepResult.risk) formatted.risk = stepResult.risk;
  if (stepResult.reasons) formatted.reasons = stepResult.reasons;
  if (stepResult._execution_path) formatted._execution_path = stepResult._execution_path;
  if (stepResult.security_warnings) formatted.security_warnings = stepResult.security_warnings;
  if (stepResult.executed_with_warning) formatted.executed_with_warning = true;
  if (stepResult.dangerous_env_keys) formatted.dangerous_env_keys = stepResult.dangerous_env_keys;
...
export function formatMacroResult(macroResult, mode = 'summary') {
  ...
  // v0.1.12: Propagate approval/policy details to MCP output
  if (macroResult.approval_required) {
    formatted.approval_required = macroResult.approval_required;
  }
  if (macroResult.policy_denied) {
    formatted.policy_denied = macroResult.policy_denied;
  }
  if (macroResult.approval_details) {
    formatted.approval_details = macroResult.approval_details;
  }
