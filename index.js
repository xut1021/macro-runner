...
async function handleRunMacro(args) {
  ...
  // If template is specified, load and resolve it
  if (args.template) {
        ...
        content: [
          {
            ...
            text: JSON.stringify({
              ok: false,
              error: `Unknown template: "${args.template}". Available: ${available || '(none)'}. Use list_macros to see all templates.`,
            }, null, 2),
          },
        ],
        isError: true,
    ...
    // Resolve template with overrides to generate steps
        ...
        content: [
          {
            ...
            text: JSON.stringify({
              ok: false,
              error: `Template parameter error: ${resolved.error}`,
              missing_parameters: resolved.missing,
              template_parameters: Object.keys(template.parameters || {}),
            }, null, 2),
          },
        ],
        isError: true,
    ...
    // Use the resolved steps (overrides inline steps if both provided)
      ...
      content: [
        {
          ...
          text: JSON.stringify({
            ok: false,
            error: 'Either "steps" (array of step objects) or "template" (macro template name) must be provided.',
          }, null, 2),
        },
      ],
      isError: true,
      ...
      content: [
        {
          ...
          text: JSON.stringify({
            ok: false,
            error: `${invalidSteps.length} step(s) missing required "type" field. Each step must have a type (edit, write, shell, read, conditional, assert).`,
          }, null, 2),
        },
      ],
      isError: true,
...
async function main() {
  ...
  console.error('[macro-runner] Macro Runner MCP Server v1.0.0 started');
